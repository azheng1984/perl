 ...
 *  On input:
 *	s   points to a NULL terminated string that begins with 'o', and the
*	    previous character was a backslash.
 *	uv  points to a UV that will hold the output value, valid only if the
 *	    return from the function is TRUE
 *	len is the number of bytes to add to 's' to point to the first position
 *	    in the string that the caller needs to pay attention to.  Upon
 *	    success that means it points to the first (perhaps only) byte of
 *	    the next character remaining to be processed; on failure it is the
 *	    first byte of the first erroneous character, if any.  For some
 *	    errors where the problem isn't an erroneous character, but
 *	    something missing, the pointer will be to the beginning of the
 *	    final good character.  The caller can then treat errors uniformly
 *	    if they just want to print out the whole string with a context
 *	    pointer, without this routine having to know if the string is UTF-8
 *	    or not, as the examples in regcomp.c illustrate. 
 ...
 */
	...
	*error_msg = "Missing braces on \\o{}";
    ...
    (*s)++;    /* Point to expected first digit (could be first byte of utf8 sequence if not a digit) */
 ...
 *  from; , otherwise TRUE.  In either case the caller should look at *len
 *  On input:
 *	s   points to a NULL terminated string that begins with 'x', and the
 *	    was a backslash.
 *	uv  points to a UV that will hold the output value, valid only if the
 *	    return from the function is TRUE
 *	len when added to s, yields the position of the first byte in the
 *	    string beyond the point that was handled by this function.  On
 *	    success that means it points to the first (perhaps only) byte of
 *	    the next character remaining to be processed; on failure it is the
 *	    first byte of the first erroneous character.
 *      error_msg is a pointer that will be set to an internal buffer giving an
 *	    error message upon failure (the return is FALSE).  Untouched if
 *	    function succeeds
 *	output_warning says whether to output any warning messages, or suppress
 ...
 *	    fail instead of warn or be silent.  For example, it requires 2
